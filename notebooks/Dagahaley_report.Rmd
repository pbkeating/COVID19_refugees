...
\begin{center}
The Potential Impact of COVID-19 in the `r print(location_name)` 
Shaun Truelove, Natalya Kostandova, Andrew Azman 
        
`r print(format(as.Date(Sys.Date()), "%d %B %Y") )` 
\end{center}
...
```{r table1_knitr, echo=FALSE}
#t1 = read_csv(table1, file.path("results", project, "Table1.csv"))
t1 = table1
...
t2_new = table2
t2_low = t2_new %>% filter(scenario=="Low Transmission")
t2_mid = t2_new %>% filter(scenario=="Moderate Transmission")
#### REMEMBER TO DELETE THIS AFTER SHAUN FIXES TABLE 2
t2_mid = t2_mid[-c(4),]
t2_high = t2_new %>% filter(scenario=="High Transmission")
t2 = data.frame(t2_low, t2_mid, t2_high)
t2 = t2[-c(2,8,9,15,16)]
row_labels = c(paste0("Low, $(R_0=1.5-2.0)$", "Moderate $(R_0 = 2.0 - 3.0)$", "High $(R_0 = 3.0 - 5.0)$"))
...
knitr::kable(t2, 
      caption = "Table 2. Cumulative infections, hospitalizations, intensive care unit (ICU) admissions, patients requiring ventilation, and deaths at 1, 3, and 6 months following successful introduction (i.e. in simulations where an outbreak occurs).", escape=FALSE) %>%
  kable_styling(c("basic", "scale_down")) %>%
...
``````{r fig1, echo=FALSE, message = F, warning = F, fig.fullwidth=TRUE}
pr_age10 <- read_csv(file.path("data", "age_dagahaley.csv"))

# Load the proportion severe in the camp
p_severe_dagahaley <- load(file=file.path("results", project, "p_severe_dagahaley.RData"))
